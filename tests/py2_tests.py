...
class test_sha3(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        for message, hex_digest in self.vectors_extended:
            ...
            self.assertEqual(binascii.hexlify(copy.digest()), copy_orig_digest)
            self.assertEqual(self.h.hexdigest(), hex_digest)
            self.assertEqual(binascii.hexlify(self.h.digest()), hex_digest)
